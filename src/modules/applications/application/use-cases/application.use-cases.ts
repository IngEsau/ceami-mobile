...
import { assertCanSubmitApplication } from '../../domain/application.rules';
...
export const updateGeneralData = async (repository: ApplicationRepository, application: CreditApplication, data: ApplicantGeneralData) => {
  const next = touch({ ...application, status: 'form_in_progress', applicantName: data.clientName, generalData: data });
...
export const submitApplication = async (repository: ApplicationRepository, application: CreditApplication, signatureAccepted: boolean) => {
  assertCanSubmitApplication(application, signatureAccepted);
  const next = touch({ ...application, status: 'submitted' });
  await repository.save(next);
  return next;
};
