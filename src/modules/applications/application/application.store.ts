...
import { createApplicationDraft, getApplications, updateApplicationDocuments, updateFamilyData, updateGeneralData, updateReferencesData, updateWorkData, captureSignature, submitApplication } from './use-cases/application.use-cases';
...
export const useApplicationStore = create<ApplicationState>((set, get) => ({
  ...
  submit: async () => { const application = get().currentApplication; if (!application) return undefined; const next = await submitApplication(get().repository, application); set({ currentApplication: next, applications: get().applications.map((item) => item.id === next.id ? next : item) }); return next; },
}));
